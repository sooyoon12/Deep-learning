...
def nn_forward_backward(
    ...
    reg_loss = reg * (torch.sum(W1 ** 2) + torch.sum(W2 ** 2))
...
def find_best_net(
    ...
    lr_values = hyperparams[0]
    hidden_layer_sizes = hyperparams[1]
    reg_values = hyperparams[2]
    decay_values = hyperparams[3]
    total_combinations = len(lr_values) * len(reg_values) * len(hidden_layer_sizes) * len(decay_values)

    # Ensure the number of hyperparameter combinations is reasonable
    if total_combinations > 25:
        raise ValueError("Too many combinations: limit to 25 or fewer.")
    elif total_combinations < 5:
        raise ValueError("Too few combinations: provide at least 5 sets.")

    # Initialize tracking variables
    combination_index = 0
    results_log = {}
    max_iterations = 3000  # Number of iterations for training
        ...
        for reg in reg_values:
            for hidden_size in hidden_layer_sizes:
                for decay in decay_values:
                    combination_index += 1
                    print(
                        f"Training combination {combination_index}/{total_combinations}: "
                        f"lr={lr:.1e}, reg={reg:.1e}, hidden_size={hidden_size}, decay={decay:.2f}"
                    )

                    # Initialize the neural network
                    input_dimension = data_dict["X_train"].shape[1]
                    ...
                    # Train the model and collect statistics
                    ...
                    predictions = model.predict(data_dict["X_val"])
                    validation_accuracy = (
                        (predictions == data_dict["y_val"]).double().mean().item() * 100.0
                    )
                    print(f"Validation accuracy: {validation_accuracy:.2f}%")

                    # Update best model if current combination performs better
                    if validation_accuracy > best_val_acc:
                        best_val_acc = validation_accuracy
                        best_stat = training_stats
                        best_net = model
                        print(f"New best accuracy: {best_val_acc:.2f}%")

                    # Log the results for analysis
                    results_log[(lr, reg, hidden_size, decay)] = validation_accuracy
