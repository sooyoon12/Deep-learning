...
def test_one_param_set(
    ...
    train_acc = (train_pred == data_dict['y_train']).sum().item() / data_dict['y_train'].size(0) * 100.0
    val_pred = cls.predict(data_dict['X_val'])
    val_acc = (val_pred == data_dict['y_val']).sum().item() / data_dict['y_val'].size(0) * 100.0
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = X[i].unsqueeze(0).mm(W)
        scores -= torch.max(scores)  # Numeric stability fix
        exp_scores = torch.exp(scores)
        probabilities = exp_scores / torch.sum(exp_scores)

        # Loss calculation for the correct class
        loss += -torch.log(probabilities[0, y[i]])

        # Gradient calculation
        for j in range(num_classes):
            if j == y[i]:
                dW[:, j] += (probabilities[0, j] - 1) * X[i]
            else:
                dW[:, j] += probabilities[0, j] * X[i]

    # Average loss and add regularization
    loss /= num_train
    loss += reg * torch.sum(W * W)

    # Average gradient and add regularization term
    dW /= num_train
    dW += 2 * reg * W
    #############################################################################
    #                          END OF YOUR CODE                                 #
...
def softmax_loss_vectorized(
    ...
    correct_class_probabilities = probabilities[torch.arange(num_train), y]
    loss = -torch.sum(torch.log(correct_class_probabilities)) / num_train
    loss += reg * torch.sum(W * W)


    probabilities[torch.arange(num_train), y] -= 1  
    dW = X.t().mm(probabilities) / num_train  # Shape: (D, C)
    dW += 2 * reg * W 
    #############################################################################
    #                          END OF YOUR CODE                                 #
    #############################################################################
