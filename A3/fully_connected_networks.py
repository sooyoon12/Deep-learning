...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        ##################################################
        # Replace "pass" statement with your code
           # Print shapes of calculated gradients
        db = dout.sum(dim = 0)
...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden = torch.clamp(X_flat.mm(W1) + b1, min=0)
        ...
        grads['W2'] = hidden.T.mm(dscores) + 2 * self.reg * W2
        grads['b2'] = torch.sum(dscores, dim=0)
    
        dhidden = dscores.mm(W2.T)
        dhidden[hidden <= 0] = 0  
    
        grads['W1'] = X_flat.T.mm(dhidden) + 2 * self.reg * W1
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        exp_scores = torch.exp(scores - torch.max(scores, dim=1, keepdim=True)[0])
        probs = exp_scores / torch.sum(exp_scores, dim=1, keepdim=True)
        correct_logprobs = -torch.log(probs[range(X.shape[0]), y])
        data_loss = torch.sum(correct_logprobs) / X.shape[0]
        reg_loss = 0.5 * self.reg * sum(torch.sum(self.params[f'W{i+1}']**2) for i in range(self.num_layers))
        loss = data_loss + reg_loss
        ...
        dout, grads[f'W{self.num_layers}'], grads[f'b{self.num_layers}'] = Linear.backward(dscores, caches.pop())
        grads[f'W{self.num_layers}'] += self.reg * self.params[f'W{self.num_layers}']
    
        for i in range(self.num_layers - 1, 0, -1):
            if self.use_dropout:
                dout = Dropout.backward(dout, dropout_caches.pop())
            dout, grads[f'W{i}'], grads[f'b{i}'] = Linear_ReLU.backward(dout, caches.pop())
...
def create_solver_instance(data_dict, dtype, device):
    ...
    # Replace "pass" statement with your code
    import sys
    sys.path.append('/content/drive/My Drive/A3/eecs598')
