...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out_height = 1 + (input_height + 2 * padding - filter_height) // stride
        out_width = 1 + (input_width + 2 * padding - filter_width) // stride

        # Initialize output tensor
        out = torch.zeros(
            (batch_size, num_filters, out_height, out_width),
        ...
        for i in range(batch_size):
            for j in range(num_filters):
                for h in range(out_height):
                    for w_ in range(out_width):  # Avoid reusing `w` as variable
                        h_start = h * stride
                        w_start = w_ * stride
                        x_segment = x_padded[i, :, h_start:h_start + filter_height, w_start:w_start + filter_width]
                        out[i, j, h, w_] = torch.sum(x_segment * w[j]) + b[j]  # Ensure `b` is tensor-like
    ...
    def backward(dout, cache):
        ...
        ###############################################################
        # Replace "pass" statement with your code
        x, w_filter, b, conv_param = cache  # w를 w_filter로 명확히 변경
        batch_size, in_channels, input_height, input_width = x.shape
        num_filters, _, filter_height, filter_width = w_filter.shape
        stride, padding = conv_param['stride'], conv_param['pad']
        ...
        for i in range(batch_size):
            for j in range(num_filters):
                for h in range(out_height):
                    for w in range(out_width):  # 변수 이름 w → w_
                        h_start = h * stride
                        w_start = w * stride
                        x_segment = x_padded[i, :, h_start:h_start + filter_height, w_start:w_start + filter_width]
                        dw[j] += x_segment * dout[i, j, h, w]
                        dx_padded[i, :, h_start:h_start + filter_height, w_start:w_start + filter_width] += w_filter[j] * dout[i, j, h, w]
                        db[j] += dout[i, j, h, w]
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        # Calculate output dimensions
        out_height = 1 + (input_height - pool_height) // stride
        out_width = 1 + (input_width - pool_width) // stride

        # Initialize output tensor
        out = torch.zeros((batch_size, in_channels, out_height, out_width), dtype=x.dtype, device=x.device)
            ...
            for c in range(in_channels):
                for h in range(out_height):
                    for w in range(out_width):
                        h_start = h * stride
                        w_start = w * stride
                        region = x[i, c, h_start:h_start + pool_height, w_start:w_start + pool_width]
                        out[i, c, h, w] = torch.max(region)
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
        ...
        for n in range(N):
            for h in range(H_out):
                for w in range(W_out):
                    ...
                    for c in range(C):
                        local_x = x[n, c, h_start:h_end, w_start:w_end]
                        max_val = torch.max(local_x)
                        # 부동 소수점 비교를 위한 isclose 사용
                        mask = (local_x == max_val)
                        mask = mask.to(dtype=x.dtype)  # Ensure dtype consistency
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # Replace "pass" statement with your code
        C, H, W = input_dims
        self.params['W1'] = (torch.randn(num_filters, C, filter_size, filter_size, dtype=dtype, device=device)
                             * weight_scale)
        ...
        self.params['W2'] = (torch.randn(flattened_dim, hidden_dim, dtype=dtype, device=device)
                             * weight_scale)
        self.params['b2'] = torch.zeros(hidden_dim, dtype=dtype, device=device)

        self.params['W3'] = (torch.randn(hidden_dim, num_classes, dtype=dtype, device=device)
                             * weight_scale)
    ...
    def loss(self, X, y=None):
        ...
        hidden_out, hidden_cache = Linear_ReLU.forward(conv_out.view(conv_out.size(0), -1), W2, b2)
        scores, final_cache = Linear.forward(hidden_out, W3, b3)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dhidden_out, grads['W3'], grads['b3'] = Linear.backward(dscores, final_cache)
        dconv_out, grads['W2'], grads['b2'] = Linear_ReLU.backward(dhidden_out, hidden_cache)
        dconv_out = dconv_out.view(conv_out.shape)
        _, grads['W1'], grads['b1'] = Conv_ReLU_Pool.backward(dconv_out, conv_cache)
...
class DeepConvNet(object):
    ...
    def __init__(self, input_dims=(3, 32, 32),
        ...
        filter_size = 3
        output_height, output_width = input_dims[1], input_dims[2]

        for layer_idx in range(1, self.num_layers):
            num_output_channels = num_filters[layer_idx - 1]
            num_input_channels = input_dims[0] if layer_idx == 1 else num_filters[layer_idx - 2]

            # 가중치 초기화
            weight_shape = (num_output_channels, num_input_channels, filter_size, filter_size)
            if weight_scale == 'kaiming':
                self.params[f'W{layer_idx}'] = kaiming_initializer(
                    num_input_channels, num_output_channels, 
                    K=filter_size, relu=True, 
                    dtype=dtype, device=device
                )
            else:
                self.params[f'W{layer_idx}'] = weight_scale * torch.randn(weight_shape, dtype=dtype, device=device)

            self.params[f'b{layer_idx}'] = torch.zeros(num_output_channels, dtype=dtype, device=device)

            # 배치 정규화
            if self.batchnorm:
                self.params[f'gamma{layer_idx}'] = torch.ones(num_output_channels, dtype=dtype, device=device)
                self.params[f'beta{layer_idx}'] = torch.zeros(num_output_channels, dtype=dtype, device=device)

            if layer_idx - 1 in max_pools:
                output_height //= 2
                output_width //= 2

        # Fully Connected Layer 초기화
        num_final_filters = num_filters[-1]
        input_dim_fc = num_final_filters * output_height * output_width

        if weight_scale == 'kaiming':
            self.params[f'W{self.num_layers}'] = kaiming_initializer(
                input_dim_fc, num_classes, dtype=dtype, device=device
            )
        else:
    ...
    def loss(self, X, y=None):
        ...
        # 네트워크의 총 레이어 수와 초기 입력 데이터 설정
        num_layers, current_output = self.num_layers, X
        intermediate_cache = {}
        
        # Conv 레이어와 활성화 함수, 배치 정규화, 풀링 처리
        for layer_index in range(1, num_layers):
            weight, bias = self.params[f'W{layer_index}'], self.params[f'b{layer_index}']
            
            # Conv 연산
            current_output, intermediate_cache[f'conv{layer_index}'] = FastConv.forward(
                current_output, weight, bias, conv_param)
            
            # Batch Normalization 처리
            if self.batchnorm:
                gamma, beta = self.params[f'gamma{layer_index}'], self.params[f'beta{layer_index}']
                current_output, intermediate_cache[f'bn{layer_index}'] = SpatialBatchNorm.forward(
                    current_output, gamma, beta, self.bn_params[layer_index - 1])
            
            # ReLU 활성화 함수 적용
            current_output, intermediate_cache[f'relu{layer_index}'] = ReLU.forward(current_output)
            
            # Max Pooling 처리
            if layer_index - 1 in self.max_pools:
                current_output, intermediate_cache[f'pool{layer_index}'] = FastMaxPool.forward(
                    current_output, pool_param)
        
        # Fully Connected (Linear) Layer 처리
        final_weight, final_bias = self.params[f'W{num_layers}'], self.params[f'b{num_layers}']
        scores, intermediate_cache[f'lin{num_layers}'] = Linear.forward(
            current_output, final_weight, final_bias)
        ...
        dout, dw, db = Linear.backward(dout, intermediate_cache[f'lin{L}'])
        ...
        for l in reversed(range(1, L)):
            ...
            if l - 1 in self.max_pools:
                dout = FastMaxPool.backward(dout, intermediate_cache[f'pool{l}'])

            # propagate through ReLU layer
            dout = ReLU.backward(dout, intermediate_cache[f'relu{l}'])

            if self.batchnorm:
                dout, dgamma, dbeta = SpatialBatchNorm.backward(dout, intermediate_cache[f'bn{l}'])
                grads[f'gamma{l}'] = dgamma
                grads[f'beta{l}'] = dbeta

            # propagate through conv layer
            dout, dw, db = FastConv.backward(dout, intermediate_cache[f'conv{l}'])
            grads[f'W{l}'], grads[f'b{l}'] = dw, db
...
def create_convolutional_solver_instance(data_dict, dtype, device):
    ...
    model = DeepConvNet(
        ...
        num_classes=10,
        num_filters=[32, 64,128],  # 필터 크기 증가
        max_pools=[0, 1, 2],
        batchnorm=True,  # 배치 정규화 활성화
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        dmean = (dx_normalized * -1 / torch.sqrt(sample_var + eps)).sum(dim=0) + \
                dvar * (-2 * (x - sample_mean).sum(dim=0) / N)
    ...
    def backward_alt(dout, cache):
        ...
        dx = (gamma / N) * (sample_var + eps)**-0.5 * (
            N * dout - dout.sum(dim=0) - x_normalized * (dout * x_normalized).sum(dim=0))
