...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        for j in range(num_test):
            ...
            dists[i,j]=torch.sum((train_flat-test_flat)**2)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        ...
        dists[i]=torch.sum((test_flat-train_flat)**2,dim=1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train_squared = torch.sum(x_train_flat ** 2, dim=1, keepdim=True)
    x_test_squared = torch.sum(x_test_flat ** 2, dim=1, keepdim=True).T
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for j in range(num_test):
         ...
         closest_labels = y_train[topk_indices]
         y_pred[j] = torch.bincount(closest_labels).argmax()
...
def knn_cross_validate(
    ...
    for k in k_choices:
        for i in range(num_folds):
            ...
            y_val_pred = knn.predict(x_val_fold, k)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, accuracies in k_to_accuracies.items():
        ...
        if mean_accuracy > best_accuracy or (mean_accuracy == best_accuracy and (best_k is None or k < best_k)):
