...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    multiples = [n for n in range(start, stop + 1) if n % 10 == 0]
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    first_half = x_reshaped[:, :4]
    second_half = x_reshaped[:, 4:]
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = x.clone()
    min_vals, min_indices = torch.min(x, dim=1)
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    B = x.size(0)
    z = torch.zeros((B, x.size(1), y.size(2)), dtype=x.dtype, device=x.device)
    for i in range(B):
...
def normalize_columns(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    col_mean = x.sum(dim=0) / x.size(0)
    col_var = ((x - col_mean) ** 2).sum(dim=0) / x.size(0)
...
def challenge_mean_tensors(xs: List[Tensor], ls: Tensor) -> Tensor:
    ...
    index = torch.repeat_interleave(torch.arange(len(xs), device=concatenated.device), ls)
    segment_sum = torch.zeros(len(xs), device=concatenated.device).scatter_add(0, index, concatenated)
    y = segment_sum / ls
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def challenge_get_uniques(x: torch.Tensor) -> Tuple[Tensor, Tensor]:
    ...
    diff = torch.cat((torch.tensor([True], device=x.device), sorted_x[1:] != sorted_x[:-1]))
    uniques = sorted_x[diff]
    indices = sorted_indices[diff]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
